...
class Contacts
{
     ...
     */
    public function add($companyID, $firstName, $lastName, $nameTitle, $title, $department,
        $reportsTo, $email1, $email2, $phoneWork, $phoneCell, $phoneOther, $address,
        $city, $state, $zip, $isHot, $notes, $enteredBy, $owner, $technologies, $web_profile, $call_plan, $IsHiringMgr = 0)
    {
        ...
        $sql = sprintf(
            ...
            $this->_db->makeQueryString($nameTitle),
            ...
            $this->_siteID,
			$this->_db->makeQueryString($technologies),
			$this->_db->makeQueryString($web_profile),
			$this->_db->makeQueryInteger($call_plan),
			$this->_db->makeQueryInteger($IsHiringMgr)
        );
     ...
     */
    public function update($contactID, $companyID, $firstName, $lastName, $nameTitle,
        $title, $department, $reportsTo, $email1, $email2, $phoneWork, $phoneCell,
        $phoneOther, $address, $city, $state, $zip, $isHot,
        $leftCompany, $notes, $owner, $email, $emailAddress, $technologies, $web_profile, $call_plan, $IsHiringMgr = 0)
    {
        /* Get the department ID of the selected department. */
        $departmentID = $this->getDepartmentIDByName(
            $department, $companyID, $this->_db
        ...
        $sql = sprintf(
            ...
            SET
				contact.call_plan	  = %s,
				contact.web_profile   = %s,
				contact.technologies  = %s,
                contact.company_id    = %s,
                contact.first_name    = %s,
                contact.last_name     = %s,
                contact.name_title    = %s,
                contact.title         = %s,
                ...
                contact.IsHiringMgr = %s
            WHERE
                contact.contact_id = %s
            AND
                contact.site_id = %s",
			$this->_db->makeQueryString($call_plan),
			$this->_db->makeQueryString($web_profile),
			$this->_db->makeQueryString($technologies),
            $this->_db->makeQueryInteger($companyID),
            $this->_db->makeQueryString($firstName),
    ...
    public function get($contactID)
    {
        $sql = sprintf(
            "SELECT
                ...
                contact.name_title AS nameTitle,
				...
				contact.technologies AS technologies, 
				contact.web_profile AS web_profile,
				contact.call_plan as call_plan, 
				contact.IsHiringMgr as IsHiringMgr 
            FROM
                contact
            LEFT JOIN company
	...
	public function getFullName($contactID)
    {
        $sql = sprintf(
     ...
     */
    public function getAll($userID = -1, $companyID = -1, $contactID = -1)
    {
        if ($userID >= 0)
        {
		...
		if ($contactID >= 0)
        {
            $contactCriterion = sprintf(
                'AND contact.contact_id = %s', $contactID
            );
        }
        else
        {
	...
	public function getEmailListingByAlpha($alpha, $userID = -1)
    {
        if ($userID >= 0)
        {
            $userCriterion = sprintf(
		...
		$userCriterion .= "AND (contact.last_name LIKE '$alpha%' OR contact.first_name LIKE '$alpha%')";
