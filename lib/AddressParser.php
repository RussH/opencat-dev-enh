...
class AddressParser
{
    ...
    protected static function _isCityStateZip($string)
    {
		...
		*/
		$cityStateZip = '[a-z\s.-]+[;,\s-]+(?:(?:'
            . implode('|', $statePrefixes)
            . ')\s+)?[a-z]{2,}[;.,\s-]+\d{5}(-\d{4})?|(\w{6})|(\w{3}\s{1}\w{3})';
            ...
            !preg_match('/^' . $POBox . '/i', $string))
        {
			...
			//added for canadian address validation
			$parts = explode($string, " ");
			if(count($parts) >=3) {
				$ubound = count($parts);
				if(strlen(trim($parts[$ubound-1]))==3 && strlen(trim($parts[$ubound-2]))==3)
					return true;
			}
    ...
    protected function _getCityStateZipArray($cityStateZipLine)
    {
        ...
        else
        {
            ...
            else
            {
				...
				if(strlen($tokens[$tokenCount-1]) == 3 &&  strlen($tokens[$tokenCount-2]) == 3) {
					//must be canadian postal code
					if(ctype_alnum($tokens[$tokenCount-1]) && ctype_alnum($tokens[$tokenCount-2])) {
						//its a canadian post code, 2 words, 3 chars each
						$city  = ArrayUtility::implodeRange(
							' ', $tokens, 0, ($tokenCount - 4)
						);
						$state = $tokens[$tokenCount - 3];
						$zip   = $tokens[$tokenCount - 2]." ".$tokens[$tokenCount - 1];	
					} else {
						$city  = ArrayUtility::implodeRange(
							' ', $tokens, 0, ($tokenCount - 3)
						);
