...
class Users
{
	 ...
	 * Modified by Allan, 12/13/2012
	 * To Associate the user login to a candidate record
	 * via the candidate_id field
     */
    public function add($lastName, $firstName, $email, $username, $password,
        $accessLevel, $eeoIsVisible = false, $candidate_id = '')
    {
        $sql = sprintf(
            "INSERT INTO user (
                user_name,
	 ...
	 * to add the candidate_id field update
     */
    public function update($userID, $lastName, $firstName, $email,
                           $username, $accessLevel = -1, $eeoIsVisible = false, $candidate_id = '')
    {
    ...
    public function get($userID)
    {
        $sql = sprintf(
            "SELECT
				...
				user.candidate_id as candidate_id,
                DATE_FORMAT(
                    MAX(
     ...
     */
	 /*
	   Modified by Allan 2/13/2012
	   To convert username automatically to lower case
    ...
    public function getIDByUsername($username)
    {
        $sql = sprintf(
            ...
            AND
                site_id = %s",
            $this->_db->makeQueryString(strtolower($username)),
            $this->_siteID
        );
	...
	//to get UserID by Candidate ID
	public function getIDByCandidateID($candidate_id)
    {
        $sql = sprintf(
            "SELECT
                user_id AS userID
            ...
            AND
                site_id = %s",
            $this->_db->makeQueryInteger($candidate_id),
            $this->_siteID
        );
        $data = $this->_db->getAssoc($sql);
    ...
    public function usernameExists($username)
    {
        ...
        $sql = sprintf(
			"SELECT
				  COUNT(user.user_name) AS userExists, max(password) as pwd
			FROM
			  user
			WHERE
			  user.user_name = %s
    ...
    public function emailExists($email)
    {
        ...
        $sql = sprintf(
            "SELECT
                  COUNT(user.email) AS emailExists, max(password) as pwd
              FROM
                  user
              WHERE
                  user.email = %s
    ...
    /**
     * Returns the email address of a user by its username
     *
     * @return boolean user exists
     */
    public function getEmailByUsername($username)
    {
        ...
        $rs = $this->_db->getAssoc($sql);
        if (isset($rs['emailVal']) || strlen($rs['emailVal']) > 0)
        {
            return $rs['emailVal'];
        }
		return '';
