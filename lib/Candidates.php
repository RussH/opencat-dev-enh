...
include_once('./lib/Users.php'); //Added by Allan
...
class Candidates
{
     ...
     */
    public function add($firstName, $middleName, $lastName, $title, $email1, $email2,
        ...
        $skipHistory = false, $skype_id = '', $contact_type = '', $country = '', $eligibility = "",$crclassification="",$communication="",$jobzone="")
    {
        $sql = sprintf(
            ...
            $this->_db->makeQueryString($skype_id),
			$this->_db->makeQueryString($country),
			$this->_db->makeQueryString($contact_type),
			$this->_db->makeQueryString($eligibility),
			$this->_db->makeQueryString($crclassification),
			$this->_db->makeQueryString($communication),
			$this->_db->makeQueryString($jobzone)
        );
	...
	/*
	  Added by Allan, 2/13/2012
	  Add a candidate with a "candidate-level" user account login
	*/
	public function add_candidate_as_a_user($site_id, $candidate_id, $email, 
		$first_name, $last_name, $phone_work, $phone_cell, $phone_other, $address, 
		$notes, $company, $city, $state, $zip_code) {
		
		$users	= new Users($site_id);
		//only add login record when email exists
		if(strlen($email."")>0) {
			$send_notification_email = false;
			$user_id = $users->getIDByUsername($email);			
			if($user_id == false) {
				//the candidate's user login record is not yet existing, add it
				$password = md5(uniqid(rand(), true));
				$password = substr($password, 0, 8); //generate a random 8-length string password				
				$user_id = $users->add($last_name, $first_name, $email, 
									   $email, $password,
        							   ACCESS_LEVEL_CANDIDATE, 
									   false, 
									   $candidate_id);
				$send_notification_email = true;
			} else {
				//user login record already exists, just update
				$update_result = $users->update($user_id, 
				                                $last_name, $first_name, $email,
                           				  		$email, 
												ACCESS_LEVEL_CANDIDATE,  
												false,
												$candidate_id);	
				if(!$update_result) $user_id = -1;
			}
			if($user_id > -1) {
				//Send notification email to the user of this account
				if($send_notification_email == true) {
					$mailer = new Mailer($site_id);
					$email_body = '
						Hello '.$first_name.', 
						<br><br>
						Your account for ClearRoad IT candidate portal has been created.<br>
						You can search for current job openings directly within the portal.
						<br><br>
						You can now login at http://www.clearroad.it/myprofile using the ff. credentials:<br>
						Username: '.$email.'
						Password: '.$password.'&nbsp;&nbsp;<i>(you can change this later after logging into your portal account)</i>
						<br><br>
						ClearRoad IT
						<br><br>';
					$mailerStatus = $mailer->sendToOne(array($email, ''),
													   'Your Candidate Portal Account at ClearRoad IT',
													   $email_body,
													   true);
				}
				return $user_id;
			} else {
				die("Unable to insert user. Error: ".mysql_error());
				return false;			
			}
		} else {
			die("No email defined.");
			return false;
		}
	}

    /**
     * Returns true if a user by the specified email already exists; false
     * otherwise.
     *
    ...
    public function emailExists($email)
    {
        ...
        $sql = sprintf(
            "SELECT COUNT( candidate.email1 ) AS emailExists, max( user.password ) AS pwd
			FROM user
			LEFT JOIN candidate ON candidate.candidate_id = user.candidate_id
	...
	function update_skills($candidateID, $skill_values, $skillyear_values, $skillprof_values) {
		$sql = sprintf(
            "UPDATE
                candidate
            SET                
            ...
            AND
                site_id = %s",                        
            $this->_db->makeQueryString($skill_values),
			$this->_db->makeQueryString($skillyear_values),
			$this->_db->makeQueryString($skillprof_values),
     ...
     */
    public function update($candidateID, $isActive, $firstName, $middleName, $lastName, $title,
        ...
        $gender = '', $race = '', $veteran = '', $disability = '', $country = '', $skype_id = '', $contact_type = '', $eligibility = '',$communication = '',$crclassification = '', $jobzone = '' )
    {
        $sql = sprintf(
            "UPDATE
                candidate
            ...
            $this->_db->makeQueryString($currentPay),
            $this->_db->makeQueryString($desiredPay),
            $this->_db->makeQueryString($canRelocate),
            ($isHot ? '1' : '0'),
            $this->_db->makeQueryString($notes),
            $this->_db->makeQueryString($webSite),
            $this->_db->makeQueryString($bestTimeToCall),
		...
		/*
			Added by Allan, 2/13/2012
			to also update the candidate's user login record if any
		*/
		$users	= new Users($this->_siteID);
		$user_id = $users->getIDByCandidateID($candidateID);
    ...
    public function delete($candidateID)
    {
		...
		//Added by Allan, 2/13/2012
		//to Delete a candidate's user login entry if the candidate record is being deleted
		/* Delete the Candidate's User Login record if any */
		 $sql = sprintf(
            "DELETE FROM
                user 
            WHERE
    ...
    public function get($candidateID)
    {
        $sql = sprintf(
            "SELECT
				...
				candidate.skype_id as skype_id,
				candidate.contact_type as contact_type,
				candidate.eligibility as eligibility,
				candidate.communication as communication,
				candidate.crclassification as crclassification,
				candidate.jobzone as jobzone
            FROM
                candidate
	...
	public function getFullName($candidateID)
    {
        $sql = sprintf(
     ...
     */
    public function getAll($allowAdministrativeHidden = false, $companyId = -1)
    {
        if (!$allowAdministrativeHidden)
        {
            $adminHiddenCriterion = 'AND candidate.is_admin_hidden = 0';
		...
		if ($companyId>0)
        {
            $companyCriterion = "JOIN (SELECT name FROM company WHERE company_id = $companyId) AS comp_tbl
				ON candidate.current_employer = comp_tbl.name";
        }
        else
        {
            $companyCriterion = '';
        ...
        $sql = sprintf(
            ...
            FROM
                candidate
            LEFT JOIN user AS owner_user
                ON candidate.entered_by = owner_user.user_id
            %s
			WHERE
                candidate.site_id = %s
            %s
	...
	public function getEmailListingByAlpha($alpha, $allowAdministrativeHidden=true)
    {
        $adminHiddenCriterion = (!$allowAdministrativeHidden) ? 'AND candidate.is_admin_hidden = 0' : '';
        $adminHiddenCriterion .= "AND (candidate.last_name LIKE '$alpha%' OR candidate.first_name LIKE '$alpha%')";
        $sql = sprintf(
            "SELECT
                candidate.candidate_id AS id,
                candidate.last_name AS lastName,
                candidate.first_name AS firstName,
                candidate.email1 AS email1,
				'' AS email2
    ...
    public function __construct($instanceName, $siteID, $parameters, $misc = 0)
    {
        ...
        $this->_classColumns = array(
            ...
            'Key Skills' =>    array('select'  => 'candidate.key_skills AS keySkills',
                                     ...
                                     'pagerRender' => '
										$dataStrSkills = $rsData["keySkills"];
										...
										foreach($dataArrSkills as $val){
											...
											if($sp != $specialization){
												$key_skills_str .= "<br><span><i>Specialization: </i></span><h3 style=\"display: inline; width: 600px;\">".$specialization."</h3>";
												$sp = $specialization;
											}
											if($roles){
												$key_skills_str .= "<div style=\"padding-left: 30px;\"><u>".$roles;
											}
											if($roles && $yr){
												$key_skills_str .= "</u> - <b>".$yr." yr</b></div>";
											} elseif($roles) {
												$key_skills_str .= "</u> - <b>Unknown</b></div>";
											}
										}
										return $key_skills_str;',
