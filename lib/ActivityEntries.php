...
class ActivityEntries
{
	...
	public function addActivityMessage($activityEntryID, $messageID){
		$sql = sprintf(
			...
			$this->_db->makeQueryInteger($activityEntryID),
    ...
    {
		$const = '';
		if ($dataItemType == DATA_ITEM_CONTACT) $const = DATA_ITEM_MESSAGING_CONTACT;
		if ($dataItemType == DATA_ITEM_CANDIDATE) $const = DATA_ITEM_MESSAGING_CANDIDATE;
		...
		$sql = sprintf(
            ...
            LEFT JOIN company
                ON joborder.company_id = company.company_id			
			LEFT JOIN activity_message
				ON activity_message.activity_id = activity.activity_id
			LEFT JOIN messaging
				ON messaging.id = activity_message.message_id
            WHERE
                activity.data_item_id = %s
            AND
               ( activity.data_item_type = %s
				OR activity.data_item_type = %s )
            AND
                activity.site_id = %s
            ORDER BY
                dateCreatedSort ASC",
            $this->_db->makeQueryInteger($dataItemID),
            $this->_db->makeQueryInteger($dataItemType),
            $this->_db->makeQueryInteger($const),
            $this->_siteID
        );
