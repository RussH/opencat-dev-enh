...
class DataGrid
{
    ...
    {	
		...
		//To Hide the LeftMost column of the DataGrid on Candidate Accounts
        ...
        foreach ($this->_currentColumns as $index => $data)
        {
           ...
           if (isset($this->allowResizing) && $this->allowResizing == true)
           {
            ...
            if ($sizable)
            {
				...
				$cColumnsArr = array_keys($this->_currentColumns);
               if (isset($cColumnsArr) && end($cColumnsArr) != $index)
               {
                   //Uncomment for gray resize bars
                ...
                echo sprintf(
                    ...
                    $md5InstanceName,
                    // $md5InstanceName, end(array_keys($this->_currentColumns)),
                    $md5InstanceName, end($cColumnsArr),
                    $this->_tableWidth,
                    urlencode($this->_instanceName),
                    $_SESSION['CATS']->getCookie(),
                    $data['name'],
        ...
        echo ('<script type="text/javascript">setTableWidth("table'.$md5InstanceName.'", '.$this->_totalColumnWidths.', document.getElementById(\'cell'.$md5InstanceName.end($cColumnsArr).'\'), document.getElementById(\'cell'.$md5InstanceName.end($cColumnsArr).'div\'), ' . ($this->_tableWidth) . ');</script>' . "\n");
    ...
    public function printActionArea()
    {
		...
		// to hide action area on Candidate Portal Account
		if(intval($_SESSION['CATS']->getAccessLevel()) == ACCESS_LEVEL_CANDIDATE) {
			return;
		}
