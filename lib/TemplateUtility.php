...
class TemplateUtility
{
    ...
    public static function printHeaderBlock($showTopRight = true)
    {
		...
		if(intval($_SESSION['CATS']->getAccessLevel()) != ACCESS_LEVEL_CANDIDATE) {
			...
			echo '<table cellspacing="0" cellpadding="0" style="margin: 0px; padding: 0px; float: left;">', "\n";
			echo '<tr>', "\n";
			echo '<td rowspan="2"><img src="'.CANDIDATE_PORTAL_LOGO_SMALL.'" height="52" border="0" style="width:auto" alt="'.CANDIDATE_PORTAL_PAGE_TITLE.'" /></td>', "\n";
			echo '</tr>', "\n";
			echo '</table>', "\n";				
		}
        ...
        if ($showTopRight)
        {
			...
			if(intval($_SESSION['CATS']->getAccessLevel()) != ACCESS_LEVEL_CANDIDATE) {
				...
				echo '<a href="', $indexName, '?m=logout&portal=candidates">';
			}
			...
			//to not include site name on logout button text
			if(intval($_SESSION['CATS']->getAccessLevel()) != ACCESS_LEVEL_CANDIDATE) 
            	echo '<span>', $fullName, '&nbsp;&lt;', $username, '&gt;&nbsp;(', $siteName, ')</span>', "\n";
			else
				echo '<span>', $fullName, '&nbsp;&lt;', $username, '&gt;</span>', "\n";
    ...
    public static function printQuickSearch($wildCardString = '')
    {
		...
		if(intval($_SESSION['CATS']->getAccessLevel()) == ACCESS_LEVEL_CANDIDATE)
			return; //don't print quick search and recent list on candidate account
    ...
    public static function printTabs($active, $subActive = '', $forceHighlight = '')
    {
        ...
        foreach ($modules as $moduleName => $parameters)
        {
			...
			//Added by Allan: 2/8/2012
			//For Candidate Account Menu Restrictions			
			//----------- Begin ---------------------
			$restricted = false;
			if(intval($_SESSION['CATS']->getAccessLevel()) == ACCESS_LEVEL_CANDIDATE) {
				if($active === null)
					$active = 'joborders'; //this is the home module for the candidate account
				switch(strtolower($tabText)) {
					case 'dashboard':
					case 'activities':
					case 'companies':
					case 'contacts':
					...
					case 'reports':
						...
						break;	
					case 'candidates':
						$tabText = 'My Profile'; //rename candidates tab to this text
						break;
					case 'job orders':
						$tabText = 'Jobs'; //rename Job Orders tab to this text
						break;
				}
			} elseif(intval($_SESSION['CATS']->getAccessLevel()) == ACCESS_LEVEL_RESEARCHER || 
				...
				//tabs that are not viewable by admin
				switch(strtolower($tabText)) {
					case 'specialization':
						$restricted = true;
						break;
            ...
            if ($subTabs)
            {
                ...
                {	
					...
					if(intval($_SESSION['CATS']->getAccessLevel()) == ACCESS_LEVEL_CANDIDATE) {
						switch(strtolower($tabText)) {
							case 'jobs':
								switch(strtolower($subTabText)) {
									case 'add job order':
										$restricted = true;
										break;
								}
							case 'my profile': //formerly 'candidates' which was renamed to this
								$restricted = true;
								break;
							case 'settings':	
								$restricted = true;							
    ...
    private static function _printCommonHeader($pageTitle, $headIncludes = array())
    {
		...
		if(intval($_SESSION['CATS']->getAccessLevel()) != ACCESS_LEVEL_CANDIDATE) { 
			...
			echo '<title>'.CANDIDATE_PORTAL_PAGE_TITLE.' - ', $pageTitle, '</title>', "\n";
		}
