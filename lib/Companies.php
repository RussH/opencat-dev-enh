...
class Companies
{
						...
						$sic, $naics)
    {
        ...
        $sql = sprintf(
			...
			$this->_db->makeQueryString($cocountry),
			$this->_db->makeQueryString($prospecting),
			$this->_db->makeQueryString($timezone),
			$this->_db->makeQueryString($category),
			$this->_db->makeQueryString($employees),
			$this->_db->makeQueryString($revenue),
			$this->_db->makeQueryString($sic),
			$this->_db->makeQueryString($naics)
        );
    ...
    public function get($companyID)
    {
        $sql = sprintf(
            "SELECT
				...
				company.cocountry as cocountry,
				company.prospecting as prospecting,
				company.timezone as timezone,
				company.category as category,
				company.employees as employees,
				company.revenue as revenue,
				company.sic as sic,
				company.naics as naics 
            FROM
                company
	...
	public function update_category($old_category, $new_category)
    {
        $sql = sprintf(
            "UPDATE
                categories
            ...
            AND
                site_id = %s",                    
			$this->_db->makeQueryString($new_category),
			$this->_db->makeQueryString($old_category),			
            $this->_siteID
        );
		$queryResult = $this->_db->query($sql);
        if (!$queryResult)
	...
	public function update_industry($old_industry, $new_industry)
    {
        $sql = sprintf(
            "UPDATE
                industries
            ...
            AND
                site_id = %s",                    
			$this->_db->makeQueryString($new_industry),
			$this->_db->makeQueryString($old_industry),			
            $this->_siteID
        );
		$queryResult = $this->_db->query($sql);
        if (!$queryResult)
	...
	public function add_category($category)
    {
        ...
        $industryID = $this->_db->getLastInsertID();
        return $industryID;
    }
	
    public function add_industry($industry)
    {
	...
	public function getIndustries()
    {
        $sql = sprintf(
            ...
            FROM
                industries     
			WHERE
                industries.site_id = %s       
            ORDER BY
                industries.industry ASC",
            $this->_siteID
        );
	...
	public function getExport($IDs)
    {
        if (count($IDs) != 0)
        {
            ...
            $criterion = 'AND company.company_id IN ('.implode(',', $IDsValidated).')';
        }
        else
        {
            $criterion = '';
        ...
        $sql = sprintf(
            "SELECT
                ...
                company.name AS name,
                company.phone1 AS phone1,
                company.key_technologies AS key_technologies
            FROM
                company
            WHERE
                company.site_id = %s
