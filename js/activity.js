...
function Activity_editEntry(activityID, dataItemID, dataItemType, sessionCookie)
{
    ...
    var callBack = function ()
    {
		...
		if(errorCodeNode && errorMessageNode){
        if (!errorCodeNode.firstChild || (errorCodeNode.firstChild.nodeValue != '0' && errorCodeNode.firstChild.nodeValue != '-2'))
        {
            if (errorMessageNode.firstChild)
            {
                var errorMessage = "An error occurred while receiving a response from the server.\n\n"
                                 + errorMessageNode.firstChild.nodeValue;
		if(navigator && !navigator.appName.search('chrome')){
                	alert(errorMessage);
		}
            }
	    if(navigator && !navigator.appName.search('chrome')){
		...
		var jobOrderNodes = http.responseXML.getElementsByTagName('joborder') ? http.responseXML.getElementsByTagName('joborder') : new Array();
