...
class ListsUI extends UserInterface
{
	...
	private function emailForJobSearchSelectJob($candidateIDArray = array())
    {
        ...
        $this->_template->display('./modules/lists/ConsiderSearchEmailModal.tpl');
    }
    ...
    private function showList()
    {
		...
		//goback
		$cRS = $savedLists->get_list_ids($savedListID);
		//print_r($cRS);
		//$candidateIDArray = array();
		//foreach($cRS as $rowindex => $row) {
		//	$candidateIDArray[] = $row[$rowindex];
	   //	}
		$candidateIDArray =  ResultSetUtility::getColumnValues($cRS, 'candidateID');
		$candidateIDArrayStored = $_SESSION['CATS']->storeData($candidateIDArray);
		//print_r($candidateIDArray);
		//echo("<p>".$candidateIDArrayStored."</p>");
		//die("test 2");
    ...
    private function addToListFromDatagridModal()
    {
        ...
        if(intval($_GET["onlySelected"])==1) {
            foreach ($_GET as $key => $value)
            {
                ...
                $dataItemIDArray[] = str_replace('checked_', '', $key);
            }
		}
		else if (isset($_REQUEST['candidateIDArrayStored']) && $this->isRequiredIDValid('candidateIDArrayStored', $_REQUEST, true))
        {
            $dataItemIDArray = $_SESSION['CATS']->retrieveData($_REQUEST['candidateIDArrayStored']);
		} else if($this->isRequiredIDValid('candidateID', $_REQUEST)){
            $dataItemIDArray = array($_REQUEST['candidateID']);
        } else {
        	$dataGrid = DataGrid::getFromRequest();
