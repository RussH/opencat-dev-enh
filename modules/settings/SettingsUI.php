...
class SettingsUI extends UserInterface
{
    ...
    public function handleRequest()
    {
	    ...
	    case 'specializationPanel':
                $this->specializationPanel();
                break;
			...
			case 'customizeIndustryValues':
                if ($this->isPostBack())
                {
                    $this->oncustomizeIndustryValues();
                }
                else
                {
                    $this->customizeIndustryValues();
                }
                break;
			
			case 'customizeCategoryValues':
                if ($this->isPostBack())
                {
                    $this->oncustomizeCategoryValues();
                }
                else
                {
                    $this->customizeCategoryValues();
                }
                break;
	...
	/* --------------------------------------------------------------------------------------------------------- */
	private function customizeCategoryValues() {
		if ($this->_realAccessLevel < ACCESS_LEVEL_DEMO)
        {
            CommonErrors::fatal(COMMONERROR_PERMISSION, $this);
            return;            
        ...
        $companiesRS = $companies->getCategories();

        $this->_template->assign('extraFieldSettingsCompaniesRS', $companiesRS);
        $this->_template->assign('active', $this);
        $this->_template->display('./modules/settings/CustomizeCategoryValues.tpl');
	}
	
	private function oncustomizeCategoryValues() {
        if ($this->_realAccessLevel < ACCESS_LEVEL_SA)
        {
            CommonErrors::fatal(COMMONERROR_PERMISSION, $this);
            return;
        ...
        foreach($extraFieldsMaintScriptArray as $index => $commandEncoded)
        {
            ...
            switch ($args[0])
            {
                case 'ADDFIELD':
					...
					$insert_id = $companies->add_category(trim(urldecode($industry)));
                    break;
                ...
                case 'DELETEFIELD':
					...
					$industry = trim(urldecode($industry));
					$companies->delete_category($industry);
                    break;
                ...
                case 'RENAMEROW':
					...
					$args2 = explode(':', $industry);	
					
					$old_industry = $args2[0];
					$new_industry = $args2[1];
					
					$qresult = $companies->update_category($old_industry, $new_industry);
                    break;
            }

        }

        CATSUtility::transferRelativeURI('m=settings&a=customizeCategoryValues');		
	}
	
	/* --------------------------------------------------------------------------------------------------------- */
	
	private function customizeIndustryValues() {
		if ($this->_realAccessLevel < ACCESS_LEVEL_DEMO)
        {
            CommonErrors::fatal(COMMONERROR_PERMISSION, $this);
            return;            
        ...
        $companies = new Companies($this->_siteID);
        $companiesRS = $companies->getIndustries();

        $this->_template->assign('extraFieldSettingsCompaniesRS', $companiesRS);
        $this->_template->assign('active', $this);
        $this->_template->display('./modules/settings/CustomizeIndustryValues.tpl');
	}
	
	private function oncustomizeIndustryValues() {
        if ($this->_realAccessLevel < ACCESS_LEVEL_SA)
        {
            CommonErrors::fatal(COMMONERROR_PERMISSION, $this);
            return;
        ...
        foreach($extraFieldsMaintScriptArray as $index => $commandEncoded)
        {
            ...
            switch ($args[0])
            {
                case 'ADDFIELD':
					...
					$insert_id = $companies->add_industry(trim(urldecode($industry)));
                    break;
                ...
                case 'DELETEFIELD':
					...
					$industry = trim(urldecode($industry));
					$companies->delete_industry($industry);
                    break;
                ...
                case 'RENAMEROW':
					...
					$qresult = $companies->update_industry($old_industry, $new_industry);
                    /* 
        ...
        CATSUtility::transferRelativeURI('m=settings&a=customizeIndustryValues');		
	}
    ...
    private function specializationPanel()
    {
        // Log message that shows up on the top of the list page
        $topLog = '';

        $dataGridProperties = DataGrid::getRecentParamaters("specialization:specializationListByViewDataGrid");
        ...
        $dataGrid = DataGrid::get("specialization:specializationListByViewDataGrid", $dataGridProperties);

        $specializations = new Specialization($this->_siteID);
        $this->_template->assign('totalSpecializations', $specializations->getCount());
    ...
    private function onChangePassword()
    {
        ...
        if ($logout)
        {	
			//Modified by Allan, 2/13/2012
			//to redirect logout to proper portal candidate URL upon logout
