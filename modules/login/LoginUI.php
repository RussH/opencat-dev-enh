...
class LoginUI extends UserInterface
{
    ...
    private function showLoginForm()
    {
		...
		$this->_template->assign('forgot_msg', '');
	...
	private function onSendPassword()
    {
		$username = $this->getTrimmedInput('username', $_POST);
        $user = new Users($this->_siteID);
		$pwd = trim($user->usernameExists($username));
		if(strlen($pwd) <= 0){
			$pwd = trim($user->emailExists($username));
			$email = strlen($pwd) > 0 ? $username : '';
			if(strlen($pwd) <= 0){
				$candidates = new Candidates($this->_siteID);
				$pwd = trim($candidates->emailExists($username));
				$email = strlen($pwd) > 0 ? $username : '';
			} 
		} else {
        ...
        if (strlen($pwd)>0)
        {
	    $email_body = sprintf('<div>Someone has requested a password to the Clear Road IT Candidate Portal, <b>associated with</b> this E-mail address.</div>
<div>
	<blockquote style="margin:0 0 0 40px;border:none;padding:0px">
		<div>Your password is:&nbsp;<b>%s</b></div>
	</blockquote>
</div>
<div>When you log into this Candidate Portal, (<a target="_blank" href="http://www.clearroad.it/cats/index.php?m=login&amp;portal=candidates">here</a>) you can:</div>
<div>
	<ul>
		<li>Apply for open jobs</li>
		<li>Update your resume</li>
		<li>Update your contact information</li>
		<li>Update your skills table</li>
		<li>Update your photo</li>
	</ul>
	<div>If you have any questions regarding this Portal, or this E-mail, please call <a href="#">(403) 668-9200</a> or send a message to <a href="mailto:mail@clearroad.it"><b>mail@clearroad.it</b></a></div>
<div>
	<div title="signature">
		<div title="signature">
			<hr>
			<p>
				<span style="font-family:\'book antiqua\',palatino;font-size:medium; float: left;"><span style="color:#6bbd46"><strong>Clear Road Team</strong></span>&nbsp;|&nbsp;<span style="font-size:small"><a href="#">(403) 668-9200</a>&nbsp;|&nbsp;<a target="_blank" href="http://www.clearroad.it/" style="color:#1155cc">site</a>&nbsp;|&nbsp;<a target="_blank" href="http://ca.linkedin.com/in/allanjones" style="color:#1155cc">profile</a>&nbsp;|&nbsp;<span style="color:#1155cc"><a target="_blank" href="http://www.clearroad.it/cats/careers/index.php?m=careers&amp;p=showAll" style="color:#1155cc">jo<wbr></wbr>bs</a>&nbsp;</span>| <a target="_blank" href="http://www.clearroad.it/cats/careers/index.php?p=showJob&amp;ID=25">upload</a>&nbsp;| <a target="_blank" href="http://www.clearroad.it/cats/index.php?m=login&amp;portal=candidates">portal</a></span>
				<br><br><br><br><br>
			</p>
		</div>
	</div>
            ...
            $mailerStatus = $mailer->sendToOne(
                array($email, ''),
                "Your ClearRoad IT Password",
                $email_body,
                true,
		true,
		array('mail@clearroad.it')
            );

            if ($mailerStatus)
            {
                $this->_template->assign('forgot_msg', 'Password sent to your email.');                
            }
            else
            {
                $this->_template->assign('forgot_msg', 'Unable to send password to address specified.');
            }
        }
        else
        {
            $this->_template->assign('forgot_msg', 'No such username found.');
        }
    }
