...
class ExportUI extends UserInterface
{
    ...
    public function onExport()
    {
        ...
        if (!$this->isRequiredIDValid('dataItemType', $_POST) && !$this->isRequiredIDValid('dataItemType', $_GET))
        {
            CommonErrors::fatal(COMMONERROR_BADFIELDS, $this, 'Invalid data item type.');
        }

        $dataItemType = isset($_POST['dataItemType']) ? $_POST['dataItemType'] : $_GET['dataItemType'];

        /* Are we in "Only Selected" mode? */
        if ($this->isChecked('onlySelected', $_POST))
        {
        ...
        else
        {
            ...
            $tempIDs = $this->getTrimmedInput('ids', $_POST);
            if (!empty($tempIDs))
            {
                $IDs = explode(',', $tempIDs);
            }
