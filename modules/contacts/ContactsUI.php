...
class ContactsUI extends UserInterface
{
    ...
    public function __construct()
    {
        ...
        $this->_subTabs = array(
			...
			'Messaging'  	  => CATSUtility::getIndexName() . '?m=contacts&amp;a=emailSearchSendEmail'
        );
	...
	{	/*
		...
		//$candidateIDArray
		$recipient		= isset($_POST['recipient']) ? $_POST['recipient'] : ''; //$jobOrderID 	= $_REQUEST['jobOrderID'];
		...
		if(intval($stage) == 0) { //EDIT STAGE
			...
			$contact_array = $contacts->getEmailListing(-1);
        	...
        	$this->_template->display('./modules/contacts/EmailForJobSearchModal.tpl');
	...
	{	/*
		...
		if(intval($stage) == 0) { //EDIT STAGE
        	...
        	$this->_template->display('./modules/contacts/Messaging.tpl');
	...
	private function SendSearchEmail($email_template, $email_subject, $preview_mode, $recipient_id, $returned_recipient) {
		...
		$contacts = new Contacts($this->_siteID);
		//$contact_array = $contacts->getAll(-1, -1);
		...
		*/
		...
		$contact_data_array = $contacts->getAll(-1, -1, $recipient_id);
		...
		$replacementStrings = array(
			...
			'<a href="http://' . $_SERVER['HTTP_HOST'] . substr($_SERVER['REQUEST_URI'], 0, strpos($_SERVER['REQUEST_URI'], '?')) . '?m=contacts&amp;a=show&amp;contactID=' . $contact_data["contactID"] . '">'.
                        'http://' . $_SERVER['HTTP_HOST'] . substr($_SERVER['REQUEST_URI'], 0, strpos($_SERVER['REQUEST_URI'], '?')) . '?m=contacts&amp;a=show&amp;contactID=' . $contact_data["contactID"] . '</a>',
		    $signature 
		);		
    ...
    private function show()
    {
		...
		$data['IsHiringMgr'] = ($data['IsHiringMgr'] == 1) ? 'yes' : 'no';
    ...
    private function onAdd()
    {
        ...
        $nameTitle  = $this->getTrimmedInput('nameTitle', $_POST);
		...
		$technologies      = $this->getTrimmedInput('technologies', $_POST);
		$web_profile      = $this->getTrimmedInput('web_profile', $_POST);
		$call_plan      = $this->getTrimmedInput('call_plan', $_POST);

        $IsHiringMgr = $this->isChecked('IsHiringMgr', $_POST);
        ...
        $contactID = $contacts->add(
            $companyID, $firstName, $lastName, $nameTitle, $title, $department, $reportsTo,
            $email1, $email2, $phoneWork, $phoneCell, $phoneOther, $address,
            $city, $state, $zip, $isHot, $notes, $this->_userID, $this->_userID, $technologies, $web_profile, $call_plan, $IsHiringMgr 
        );
    ...
    private function edit()
    {
        ...
        $this->_template->assign('active', $this);
        $this->_template->assign('data', $data);
        $this->_template->assign('nameTitle', $data['nameTitle']);
    ...
    private function onEdit()
    {
        ...
        if (!$contacts->update($contactID, $companyID, $firstName, $lastName, $nameTitle,
            $title, $department, $reportsTo, $email1, $email2, $phoneWork, $phoneCell,
            $phoneOther, $address, $city, $state, $zip, $isHot,
            $leftCompany, $notes, $owner, $email, $emailAddress, $technologies, $web_profile, $call_plan, $IsHiringMgr))
        {
