...
class CareersUI extends UserInterface
{
    ...
    private function careersPage()
    {
        ...
        */
        ...
        else if ($p == 'registeredCandidateProfile' && $isRegistrationEnabled)
        {
            ...
            $content = str_replace('<input-title>', '<select id="title" name="title" class="inputbox" style="width:150px;">
                                <option value=""></option>
        ...
        else if ($p == 'applyToJob' || isset($_POST[$id='applyToJobSubAction']) && $_POST[$id] != '')
        {
            ...
            $phoneWork = isset($_POST[$id='phone']) ? $_POST[$id] : '';
			...
			//$phoneWork = isset($_POST[$id='phoneWork']) ? $_POST[$id] : '';
            ...
            $template['Content'] = str_replace('<input-title>', '<select id="title" name="title" class="inputbox" style="width:150px;">
                                <option value=""></option>
                                <option value="Mr.">Mr.</option>
                                <option value="Mrs.">Mrs.</option>
                                <option value="Ms.">Ms.</option>                                
			...
			$template['Content'] = str_replace('<input-skype_id>', '<input name="skype_id" id="skype_id" class="inputBoxNormal" value="" />', $template['Content']);
			...
			$template['Content'] = str_replace('<input-phone-work>', '<input name="phoneWork" id="phoneWork" class="inputBoxNormal" value="' . $phoneWork . '" />', $template['Content']);
            ...
            $template['Content'] = str_replace('<input-resumeUploadPreview>',
                ...
                . '<input type="hidden" id="resumeLoad" name="resumeLoad" value="Upload" onclick="resumeLoadFile();" disabled /><br /> ' 
                . $attachmentHTML
			...
			$stateRow = '
						...
						<td>
							<input name=\'state\' id=\'state\' class=\'inputBoxNormal\' value=\'\' >
							<br><span style="font: normal 10px Verdana,Arial,Helvetica;">(Two letter abbreviation for Canada/US)</span>
						</td>
					</tr>			
					<tr>			
			';

			$srchStr = '                    <td class="label"><label id="stateCountryLabel" for="stateCountry">';
			$repStr = $stateRow.$srchStr;
			$template['Content'] = str_replace($srchStr, $repStr, $template['Content']);

			$countrySelectElem = '<select name="country" id="country" class="inputbox" />
																	<option value="">----</option>                               
			...
			/* country */
			$template['Content'] = str_replace('<input-country>', $countrySelectElem, $template['Content']);

			$template['Content'] = str_replace('<input name="state" id="state" class="inputBoxNormal" value="" />', $countrySelectElem, $template['Content']);
			
			$searchElem = '<input name="emailconfirm" id="emailconfirm" class="inputBoxNormal" value="" /></td>
                </tr>';
			$appendElem = '<input name="emailconfirm" id="emailconfirm" class="inputBoxNormal" value="" /></td>
                </tr>
                <tr>
                    <td class="label"><label for="skype_id">Skype ID:</label></td>
                    <td><input name="skype_id" id="skype_id" class="inputBoxNormal" value="" /></td>
                </tr>
                <tr>
                    <td class="label"><label for="web_site">LinkedIn Url:</label></td>
                    <td><input name="web_site" id="web_site" class="inputBoxNormal" value="" /></td>
                </tr>
                <tr>
                    <td class="label"><label for="web_site">Resume Photo:</label></td>
                    <td><input type="file" name="portrait" id="portrait" value="" />
						<br><span style="font: normal 10px Verdana,Arial,Helvetica;">(Portrait orientation)</span>
					</td>
                </tr>
				';
			$template['Content'] = str_replace($searchElem, $appendElem, $template['Content']);
			...
			$appendElem = '
                <tr>
                    ...
                    <td><input name="source" id="source" class="inputBoxNormal" value="" />
						<br><span style="font: normal 10px Verdana,Arial,Helvetica;">(Where did you hear about this job?)</span>
					</td>
                </tr>
				<tr>
					<td class="label"><label for="eligibility">Eligibility:</label></td>
					<td><select id="eligibility" name="eligibility" class="inputBoxNormal">
							<option value=""></option>
							<option value="Canadian Citizen">Canadian Citizen</option>
							<option value="Canadian Permanent Resident">Canadian Permanent Resident</option>
							<option value="Canadian Work Permit Holder">Canadian Work Permit Holder</option>
							<option value="Not yet eligible">Not yet eligible</option>
							<option value="US Citizen">US Citizen</option>
						</select></td>
				</tr>			
				<tr>
					<td class="label"><label for="contact_type">Contract Type:</label></td>
					<td><select id="contact_type" name="contact_type" class="inputBoxNormal">
			...
			$searchElem = '<input name="keySkills" id="keySkills" class="inputBoxNormal" value="" />';
			$appendElem = $searchElem.'<br><span style="font: normal 10px Verdana,Arial,Helvetica;">What should we know you for? (technologies, roles, etc.)</span>';
			...
			$filter = '$GA = \'<script type="text/javascript">

				  var _gaq = _gaq || [];
				  _gaq.push(["_setAccount", "UA-39356842-1"]);
				  _gaq.push(["_setDomainName", "clearroad.it"]);
		    ...
		    $html = str_replace(\'</head>\', $GA, $html);
			$searchStr = "*Email Adddress:</label>";
			$replacementStr = "*Long-term E-mail:</label>";
		    $html = str_replace($searchStr, $replacementStr, $html);
			$searchStr = "*State/Country:</label>";
			$replacementStr = "*Country:</label>";
		    $html = str_replace($searchStr, $replacementStr, $html);
			$searchStr = "*City/Province:</label>";
			$replacementStr = "*City:</label>";
		    $html = str_replace($searchStr, $replacementStr, $html);
			';
		   ...
		   $filter = '$addedStyle = \'
				<style type="text/css" media="all">
					...
					form#applyToJobForm label, td.label {
		    ...
		    $html = str_replace(\'</head>\', $addedStyle, $html);';
        ...
        else if ($p == 'showJob')
        {
            ...
            $template['page_title'] = $jobOrderData['title'];

            $isRegistered = $this->isCandidateRegistered($siteID, $template['Content - Candidate Registration']);
			
			//Added by Allan 2/3/2012 for Apply with LinkedIn Button
			$linked_in_button_script = "<script type=\"text/javascript\" src=\"http://platform.linkedin.com/in.js\">
										    api_key: jkw3i844denr
										</script>
										<script type=\"IN/Apply\" 
											  data-jobId=\"".$jobID."\"
											  data-companyName=\"".$jobOrderData['companyName']."\" 
											  data-jobTitle=\"".$jobOrderData['title']."\"               
											  data-url=\"http://www.clearroad.it/cats/careers/apply_processor.php\" 
											  data-themeColor=\"#FF9900\" 
											  data-size=\"medium\" 
											  data-showText=\"true\" 
											  data-logo=\"http://96.45.190.9/images/emaillogo.jpg\">
										</script>";
			
			//-----OLD GOOGLE ANALYTICS CODE
			/*$google_analytics = "<script type=\"text/javascript\">
								  var _gaq = _gaq || [];
								  _gaq.push(['_setAccount', 'UA-11264050-3']);
								  _gaq.push(['_trackPageview']);
			...
			$template['Content'] = str_replace('<a-applyToJob>Apply to Job</a>', '<a-applyToJob>Apply to Job</a>'.$google_analytics, $template['Content']);*/
            ...
            {
				//Modified by Allan 2/3/2012 for Apply with LinkedIn Button
                $template['Content'] = str_replace('<a-applyToJob>Apply to Job</a>', $linked_in_button_script.'<a href="'.CATSUtility::getIndexName().'?m=careers'.(isset($_GET['templateName']) ? '&templateName='.urlencode($_GET['templateName']) : '').'&p=applyToJob&ID='.$jobID.'"'.'>Apply to Job</a>', $template['Content']);
            }
        ...
        $this->_template->assign('siteName', $siteName."__33__".$careerPortalSettingsRS['useCATSTemplate']);
    ...
    {	
		//header("location test.php?candidate_id=".$candidateID);
		//exit();
		//die("PASS!");
		//goback
        $jobOrders = new JobOrders($siteID);
		...
		$web_site       = $this->getTrimmedInput('web_site', $_POST);
        ...
        $fields = array('firstName', 'lastName', 'email', 'address', 'city', 'state', 'zip', 'phone',
            'phoneHome', 'phoneCell', 'skype_id', 'country', 'phoneWork'
        );
        $storedVal = '';
        foreach ($fields as $field)
        {
        ...
        if ($candidateID !== false)
        {
			...
			// $portrait = (isset($_FILES["portrait"]) && ($_FILES["portrait"]["size"] < 1000) ) ? $this->savePortrait($_FILES,$candidate['portrait']) : '';
			if(isset($_FILES["portrait"]) && ($_FILES["portrait"]["size"] < 100000) )
			{
				$this->savePortrait($_FILES,$candidateID); 
			}
            ...
            $candidates->update(
                ...
                $lastName, $email, $email2, $phoneHome, $phoneCell, $phoneWork, $address, $city,
                $state, $zip, $source, $keySkills, '', $employer, '', '', '', $candidate['notes'],
                $web_site, $bestTimeToCall, $automatedUser['userID'], $automatedUser['userID'], $gender,
                $race, $veteran, $disability, $country, $skype_id, $contact_type, $eligibility
            );

            /* Update extra feilds */
            $candidates->extraFields->setValuesOnEdit($candidateID);
			
			//added by Allan, 12/13/2012 ---------------------------------------------------------------------------
			//to call adding of the candidate's login record
			/*
			$candidates->add_candidate_as_a_user($siteID, $candidateID, $email, 
												 $firstName, $lastName, $phone, $phoneCell, $phoneHome, $address, 
												 $candidate['notes'], $employer, $city, $state, $zip);
			*/
			//------------------------------------------------------------------------------------------------------
        }
        else
        ...
        {			
			// $portrait = (isset($_FILES["portrait"]) && ($_FILES["portrait"]["size"] < 100000) ) ? $this->savePortrait($_FILES,$candidate['portrait']) : '';
            /* New candidate. */
			...
			//to call adding of the candidate's login record
			$candidates->add_candidate_as_a_user($siteID, $candidateID, $email, 
												 $firstName, $lastName, $phoneWork, $phoneCell, $phoneHome, $address, 
												 $extraNotes, $employer, $city, $state, $zip);
			//------------------------------------------------------------------------------------------------------
        } else {
			//added by Allan, 12/13/2012 ---------------------------------------------------------------------------
			//to call adding of the candidate's login record
	...
	/************************************************************************************************
	*	Function Name : savePortrait()
	*	parameter : $_FILES
	*	output : $newFileName
	*	Created by : Ryan Barril [ Mar. 21, 2013 ]
	************************************************************************************************/
	// public function savePortrait($holder,$oldFileName = '',$candidateID = 0)
	public function savePortrait($holder,$candidateID = 0)
	{
	        $site = new Site(-1);
			$siteID = $site->getFirstSiteID();
			$allowedExts = array("gif", "jpeg", "jpg", "png");
			$extension = end(explode(".", $holder["portrait"]["name"]));
			$allowedType = array("image/gif","image/jpeg","image/jpg","image/png");
			$file_type = $holder["portrait"]["type"];
			if( in_array($file_type,$allowedType) && in_array($extension, $allowedExts) && ($holder["portrait"]["size"] < 100000) && $candidateID > 0)
			{
				if ($holder["portrait"]["error"] > 0)
				{
					return '';
				}
				else
				{
					...
					$attachmentCreator = new AttachmentCreator($siteID);
					$attachmentCreator->createFromUpload(
						DATA_ITEM_CANDIDATE, $candidateID, 'portrait', true, false
					);

					// $newFileName = date('YmdHis').rand(1,100).".".$extension;
					// if (file_exists(PORTRAIT_PATH.$newFileName))
					// {
						// $newFileName = date('YmdHis').rand(1,100).".".$extension;
					// }
					// move_uploaded_file($holder["portrait"]["tmp_name"],	PORTRAIT_PATH.$newFileName);
					// if($oldFileName != '') unlink( PORTRAIT_PATH.$oldFileName );
					// return $newFileName;
				}
			}
