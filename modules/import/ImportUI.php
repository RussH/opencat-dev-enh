    ...
    private function onImportDelimited($fileID)
    {
        ...
        switch ($dataContaining)
        {
            ...
            case 'csv':
				...
				//TODO:
				//try also semicolons if comma is not the default delimeter
				if($theFields[1] == "") {
					$theFields = explode(";", $theFields[0]);
				}
                break;
        ...
        foreach ($theFields AS $theField)
        {
            for ($i = 0; $i < count($types); $i += 2)
            {
                ...
                $lType  = strtolower($types[$i+1]);
				
				//$stat = "comparing [".$lField. "] vs [".$lType."] = ";
		...
		/*
		die("<p>(".$importInto.")</p> 
		     <p>theFields = (".print_r($theFields,true).")</p>
			 <p>types = (".print_r($types,true).")</p>
			 <p>matchingFields = (".print_r($matchingFields,true).")</p>");
		*/
