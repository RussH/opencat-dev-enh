...
else if (!isset($_GET['m']) || empty($_GET['m']))
{
    if ($_SESSION['CATS']->isLoggedIn())
    {
        ...
        //Modified by Allan: 2/8/2012 (modify Candidate Account home page)
		if ($_SESSION['CATS']->getAccessLevel() == ACCESS_LEVEL_CANDIDATE)
			ModuleUtility::loadModule('joborders'); //JobOrders is the home page for Candidate Account
		else
...
else
{
    if ($_GET['m'] == 'logout')
    {
		...
		//to properly redirect logout button to Candidates Portal Login Page
    ...
    else if (!ModuleUtility::moduleRequiresAuthentication($_GET['m']))
    {
		...
		if(isset($_POST['m']))
			ModuleUtility::loadModule($_POST['m']);
		else
    ...
    else
    {
		...
		//Modified by Allan: 2/8/2012 (modify Candidate Account home page) 
		if ($_SESSION['CATS']->getAccessLevel() == ACCESS_LEVEL_CANDIDATE && $_GET['m']=='home')
			ModuleUtility::loadModule('joborders'); //JobOrders is the home page for Candidate Account
		else { 
			if(isset($_POST['m']))
				ModuleUtility::loadModule($_POST['m']);
